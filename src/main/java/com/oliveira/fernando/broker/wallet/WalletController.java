...
import com.oliveira.fernando.broker.wallet.error.CustomError;
import com.oliveira.fernando.broker.wallet.error.FiatCurrencyNotSupportedException;
...
import static com.oliveira.fernando.broker.data.InMemoryAccountStore.ACCOUNT_ID;
...
public record WalletController(InMemoryAccountStore store) {
    ...
    public HttpResponse<RestApiResponse> withdrawFiatMoney(@Body WithdrawFiatMoney withdraw) {
        ...
        var wallet = store.withdrawToWallet(withdraw);
        LOG.debug("Withdraw to wallet: {}", wallet);
        return HttpResponse.ok().body(wallet);
    }
}
